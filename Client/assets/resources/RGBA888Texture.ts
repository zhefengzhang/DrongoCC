...
export class RGBA8888Texture extends Texture2D {
    ...
    update2Texture(): void {
        ...
        regionInfo.texOffset.x = 0;
        regionInfo.texOffset.y = 0;
        regionInfo.texExtent.width = 1024;
        regionInfo.texExtent.height = 1024;

        const gfxTexture = this.getGFXTexture();
        if (!gfxTexture) {
            return;
        }
        gfx.deviceManager.gfxDevice.commandBuffer.copyBuffersToTexture([this.__buffer],gfxTexture,[regionInfo]);
        // gfx.deviceManager.gfxDevice.copyBuffersToTexture(
